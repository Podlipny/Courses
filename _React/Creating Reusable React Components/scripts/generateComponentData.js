...
if (enableWatchMode) {
  // Regenerate component metadata when components or examples change. - pokazde kdyz se soubory zmeni
  chokidar.watch([paths.examples, paths.components]).on('change', function(event, path) {
    generate(paths);
  });
} else {
  // Generate component metadata - pouze jednou
  generate(paths);
}

function generate(paths) {
  var errors = [];
  // ziskame seznam adresaru
  var componentData = getDirectories(paths.components).map(function(componentName) {
    // zmapujeme adresar components a ziskame data o jednotlivych component adresarich
    try {
